...
L.push('');
// Match on beverage NAME only: names are unique (vintage+producer), and the catalog
// `type` can drift between DBs (e.g. pink wines re-typed to Rosé locally but not on prod).
L.push(`SELECT '!! UNMATCHED FLAVOR BEVERAGE: ' || name || ' [' || type || ']'`);
L.push('  FROM _pf f WHERE NOT EXISTS (SELECT 1 FROM beverages b WHERE b.name=f.name);');
L.push('');
L.push('UPDATE beverages SET flavor_profile =');
L.push('    (SELECT f.flavor FROM _pf f WHERE f.name=beverages.name)');
L.push("  WHERE (flavor_profile IS NULL OR flavor_profile='')");
L.push('    AND EXISTS (SELECT 1 FROM _pf f WHERE f.name=beverages.name);');
L.push('');
...
L.push(`  (SELECT 1 FROM wine_list wl JOIN beverages b ON b.id=wl.beverage_id JOIN restaurants r ON r.id=wl.restaurant_id WHERE r.slug=${q(SLUG)} AND b.name=p.bev);`);
L.push('');
...
L.push('  JOIN beverages b ON b.id=wl.beverage_id AND b.name=p.bev;');
L.push('');
L.push('COMMIT;');
L.push('');
